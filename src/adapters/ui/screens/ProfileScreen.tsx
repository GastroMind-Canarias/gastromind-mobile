...
function getAllergenEmoji(name: string): string {
  return ALLERGEN_EMOJIS[name?.toUpperCase()] || "⚠️";
}
  ...
  return (
    <Animated.View style={{ transform: [{ scale }] }}>
      ...
      >
        ...
        {active && isEditing && <Text style={styles.toolCheck}>✓</Text>}
      </TouchableOpacity>
    </Animated.View>
  );
}
...
function AllergenPillBackend({
  ...
  const emoji = getAllergenEmoji(allergen.name);
  const scale = useRef(new Animated.Value(1)).current;
...
export default function ProfileScreen() {
  ...
  const handleRemoveMember = (id: string, name: string) => {
    Alert.alert("Eliminar miembro", `¿Eliminar a "${name}" del hogar?`, [
      ...
      <View style={styles.profileHeader}>
        ...
        <View style={styles.quickStats}>
          <View style={styles.quickStat}>
            <Text style={styles.quickStatNum}>{activeToolCount}</Text>
          ...
          <View style={styles.quickStatSep} />
          <View style={styles.quickStat}>
            <Text style={styles.quickStatNum}>{activeAllergenCount}</Text>
            <Text style={styles.quickStatLabel}>Alérgenos</Text>
          </View>
        </View>
      </View>
